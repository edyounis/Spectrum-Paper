...
was not large enough due to the memory model mandating that the latency to any memory request would be no greater than latency to L2. %% to memory latencies not being sufficiently large. This is because the
...
%% Thus, a workaround to use BOOM's floating point unit to expand the speculation 
%% window using {\tt fdiv} instructions was performed. For a real attackWhen used on the dependency, this would allow the speculation
...
The BOOM implementation of this attack echos the Spectre-v1 PoC and psuedocode closely [?].
