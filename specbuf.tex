...
This speculative buffer is designed to mitigate transient execution attacks that utilize data cache side-channels.
We will assume an active adversary that is able to read speculated data from the cache. By implementing a speculative buffer that holds uncommited loads, we will circumvent the attacker. The implementation of this buffer itself can be used as a side-channel that will be addressed in Section \ref{specbuf_sidechannel}; however, this is not within our threat model. 
