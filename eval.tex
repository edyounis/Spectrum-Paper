...
\begin{tabular}{@{} *3l @{}} \toprule
    ...
    Benchmark                                 & Normal (non-SpecBuf) & SpecBuf \\ \midrule
    Non-speculative load misses to same sets  & ??                   & ??      \\
    Non-speculative load misses to diff. sets & ??                   & ??      \\ 
    MSHR evicted speculative load miss        & ??                   & ??      \\
    Dhrystone                                 & 2176 Dhrystones/s    & 2216 Dhrystones/s \\ \bottomrule
\end{tabular}
\end{table}
...
\subsection{SpecBuff Results}

We evaluated our SpecBuff implementation using three small microbenchmarks, Dhrystone and the 
...
\subsubsection{Microbenchmark Explainations and Results}
...
load is accessing a different set in the cache. In the normal version of BOOM, the MSHR's would fill up competely but then stall 
when full since the MSHR would be unable to allocate extra entries until a previous load is completed. When using the SpecBuff ...
